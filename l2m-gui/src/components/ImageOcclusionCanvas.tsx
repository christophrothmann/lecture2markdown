      ...
      <div className="absolute top-2 left-2 right-2 flex flex-wrap items-center justify-between gap-2 z-30 pointer-events-auto">
        <div className="flex items-center gap-1.5 bg-slate-900/90 backdrop-blur-md px-2.5 py-1.5 rounded-xl border border-slate-700/80 shadow-lg text-xs">
          <button
            ...
            title="Klicke und ziehe mit der Maus über Diagramme & Text"
          >
            <EyeOff className="w-3.5 h-3.5" />
            {isDrawingMode ? 'Maske zeichnen: Aktiv' : '+ Maske zeichnen'}
          </button>
          ...
          <div className="flex items-center bg-slate-800/80 p-0.5 rounded-lg border border-slate-700">
            <button
              ...
              title="Hide One: Jede Maske wird einzeln abgefragt"
            >
              Hide One
            </button>
            <button
              ...
              title="Hide All: Alle Masken bleiben während des Tests verdeckt"
            >
              Hide All
            </button>
          </div>
        ...
        {masks.length > 0 && (
          <div className="flex items-center gap-1.5 bg-slate-900/90 backdrop-blur-md px-2.5 py-1.5 rounded-xl border border-slate-700/80 shadow-lg text-xs">
            <span className="text-[11px] font-medium text-slate-300">
              {masks.length} {masks.length === 1 ? 'Maske' : 'Masken'}
            </span>
            {onClearSlideMasks && (
              <button
                type="button"
                ...
                {editingLabelId === mask.id ? (
                  <div className="flex items-center gap-1">
                    <input
                      ...
                      placeholder="Antwort / Begriff eingeben..."
                  ...
                  <button
                    ...
                    title="Beschriftung/Antwort bearbeiten"
                  >
                    {mask.label || '+ Begriff'}
                  </button>
                )}
